...
- **Propagating context to Windmill node calls has TWO shapes: dispatch-time `flow_input` vs mid-DAG result-threaded.** Identity/tracking context known at execute time (orgId, userId, runId, campaignId, brandId, featureSlug, goal, …) is passed as `flow_input.*` and auto-injected into every node's `input_transforms` by `nodeToModule`/`buildFailureModule` in `src/lib/dag-to-openflow.ts`. But context chosen MID-flow by a node (e.g. the per-run `audienceId` that campaign-service selects inside `/start-run`, AFTER the root run is created) is NOT a flow_input — it must be threaded forward from the producing node's RESULT: detect the node, then auto-inject `results.<producer_module_id>?.audienceId` into every SUBSEQUENT node (skip the producer itself to avoid a self-reference; optional-chaining makes it `undefined` for nodes before it, so no empty header). Do NOT try to stamp such values on the root `execute-workflow` run or add them to `buildCampaignAttributionContext` — that context is built at dispatch where the value is still null and would mis-stamp the untagged root run. Propagating ANY new value to downstream node HTTP calls touches THREE surfaces in lockstep: (1) the auto-inject in `dag-to-openflow.ts`, (2) the `audienceId?`-style param + header line in EVERY `scripts/nodes/*.ts` that builds identity headers (Windmill maps `input_transforms` keys → params by NAME, so position is free), and (3) the `DOWNSTREAM_HEADER_KEYS` allowlist in `src/lib/downstream-headers.ts`. (Header `x-audience-id`; shipped PR #307.)
