...
- **`SpecWatcher`** runs hourly **and does not query on a tick that finds no spec drift**. The set of services the active workflows call is cached in memory; the steady-state tick is HTTP + a hash comparison, and Postgres is read only when the cache is cold or the specs genuinely moved (which is also when the DAGs are needed). Do not lower the interval below the idle timeout, and do not reintroduce a `SELECT` at the top of the tick.
- **`PeriodicCleanup`** owns **no timer at all** — `noteWorkflowWrite()` calls `maybeRun()` from the write paths and it sweeps at most once per 24h. A retention sweep only has work when something was written, so on a quiet service it correctly never runs and the compute sleeps.
...
**What this can and cannot buy, measured 2026-08-01.** The production compute (`ep-still-mud-a1319tcx`) was still `active` continuously after all of the above, and that is **not** a poller bug: prod dispatches **~2,900–3,500 workflow runs a day**, median gap between dispatches **22.8s**, and exactly **one** gap over 300s in 24h (max 356.8s). The compute has roughly *56 seconds* of suspendable quiet per day, so the ~182 CU-h is genuine execution, not poll-induced wakefulness — removing every timer in this repo saves ~0 while campaigns are running. The loops above still matter (they are what lets a genuinely quiet deployment sleep), but do not go hunting for more compute savings here: the lever is the *volume of real runs*, not the pollers. Verify before assuming otherwise — `SELECT` the inter-arrival gaps rather than reading the code.
