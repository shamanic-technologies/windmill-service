...
- **`POST /workflows` has NO duplicate-signature guard — a re-run collides on the index and leaks a raw 500, so any script that creates workflows must carry its own idempotence.** `PUT /workflows/{id}` (the fork path) checks `(feature_slug, signature, status='active')` first and answers a clean 409 with the existing workflow's id, and `POST /workflows/create` (the LLM path) is idempotent on `(orgId, featureSlug, signature)`. `POST /workflows` does neither: it inserts straight into the partial unique index `idx_workflows_active_sig`, so a second run of the same DAG surfaces as an unhandled Postgres error, not a 409. Until that asymmetry is fixed in the route, a seeding/ops script MUST read the feature's active workflows first and skip the cells it already covers — never rely on the route to reject the duplicate. Reference impl: `scripts/seed-feedback-request-matrix.mjs` (reads `GET /workflows?featureSlug=&status=active`, derives each row's `(prompt type, model)` cell from its stored DAG, creates only what is missing). Note when deriving a cell that an ABSENT `config.body.model` means the chat-service default (`pro`), not "no model" — reading it as absent makes an already-covered cell look missing and duplicates it.
- **A prompt template's `{{variables}}` are supplied by the DAG, so a template variable no DAG maps renders unresolved in production, silently.** `blind-discovery-email-v26` declares `{{currentDate}}` and no active workflow supplies it. When adding a template, map every variable it declares in the `email-generate` node's `inputMapping`; for the date, the documented shape is a `script` node returning `{ currentDate }` (see `src/lib/prompt-templates.ts`), which is what `scripts/seed-feedback-request-matrix.mjs` does.
